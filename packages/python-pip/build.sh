...
# Overlay from upstream with the sphinx docs toolchain REMOVED:
# upstream installs docutils/myst_parser/sphinx_* to build pip's man pages,
# a heavy download+compile that repeatedly kills the CI runner. Man pages
# are not needed for our deployment.
...
termux_step_post_make_install() {
	...
	# ( pip documentation build skipped to avoid sphinx dependency tree )
